...
def process_as_strips (full_img,
                       image_path,
                       if_SR,
                       model_path,
                       selected_columns,
    ...
    for n in range(len(striplist)):
        ...
        strip_p=process_img_to_rgb(striplist[n],
                                   image_path,
                                   if_SR,
                                   model_path,
                                   selected_columns,
...
def process_img_to_rgb(img,
    ...
    final_cord=get_water_pix_coord_v2(img,correction, if_SR, img_water) #getting coordinates of water pixels
    if len(final_cord)==0:
...
def correct_baseline(img,file_path, if_SR, file_in):
    ...
    # if L1C input 
        ...
        # If input is already a GeoTIFF (e.g., GEE export), the TOA reflectance has been corrected
        # for RADIO_ADD_OFFSET, so 1000 must be added to each file..
        if os.path.isfile(file_in) and os.path.splitext(file_in)[1].lower() in (".tif", ".tiff"):
            ...
            with rasterio.open(file_in) as src:
                info = src.tags()['info']
                productid = info.split(',')[0]
                img_name = productid.split('_')[5][1:]
            del img
...
def get_water_pix_coord_v2(img,correction, if_SR, img_water):
    coordinates_water = np.where(img_water)
    waterr, waterc = coordinates_water
    coordinate_list_water = list(zip(waterr, waterc))  # where not glint

    if if_SR == False:
        ND_coordinates = np.column_stack(
            np.where(np.all((img > correction) & (img < 30000), axis=-1)))  # where not no data (in any band)

        ND_coordinates_list = list(map(tuple, ND_coordinates))
        common_coordinates_set = set(coordinate_list_water) & set(ND_coordinates_list)
        common_coordinates_list = list(common_coordinates_set)  # Convert set to list
        del ND_coordinates, ND_coordinates_list, common_coordinates_set
        gc.collect()
...
def load_model_and_predict_pixels(value_list, model_path, cord_list, if_SR):
    ...
    for chunk_value, chunk_cord in zip(chunks_value, chunks_cord):
        ...
        # df.to_csv("/home/yan/WorkSpace/pycharm_proj/optical-shallow-deep-new/examples/temp.csv", index=False, header=False)
